...
タプルと`set_elem/3`を使うと指定したインデックスへ要素をセットすることもできます:
...
`set_elem/3`は新しいタプルを返すことに注意してください．Elixirのデータ型は変更不可能なため`tuple`という変数名で保持されているオリジナルのタプルは変更されません．変更不可であることにより，特定のコードがデータ構造を変更していないか気を配る必要はまったくなく，その分簡単になっています．
